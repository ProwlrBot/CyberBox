---
title: Prowl Pattern Catalog
description: Curated, schema-validated Fabric-style prompt patterns shipped with the prowl bug-bounty pipeline.
---

# Prowl Pattern Catalog

`prowl` is the bug-bounty pipeline orchestrator that ships with CyberBox. Beyond the classic recon → scan → report flow, it ships a Fabric-style **pattern library** of curated prompts you can pipe data into. Every pattern is reviewed, schema-constrained, and exercised by a bash test.
...
```bash
prowl patterns list           # one name per line
prowl patterns list -v        # show whether each pattern has a schema + example
prowl pattern <name>          # run a pattern on stdin/args
```

Patterns live under `prowl/patterns/<name>/` and contain:
...
```bash
bash prowl/tests/test_pattern_<name>.sh
```
...
```bash
git diff main...HEAD | prowl pattern diff_for_secrets
```
...
```bash
echo "Authorization: Bearer eyJhbGciOiJub25l..." | prowl pattern jwt_audit
```
...
1. Create `prowl/patterns/<name>/system.md` with rules ending in *"Reject low-signal output"* clauses.
2. Create `prowl/patterns/<name>/schema.json` (Draft 2020-12) with `additionalProperties: false`, enum-constrained `id` fields, and `minItems`/`minLength` constraints anywhere a model could fall back to vague output.
3. Add at least one `examples/example.input.txt` + `examples/example.output.json` pair. Validate the example against the schema.
4. Add a bash test under `prowl/tests/test_pattern_<name>.sh` that sources `_pattern_test_helper.sh` and calls `run_pattern_test "<name>"`.
