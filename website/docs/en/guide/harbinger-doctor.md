...
`harbinger doctor` makes the schema validator auditable. When you run a pattern with a `schema.json`, every AI output gets validated; outputs that fail validation are *recorded* — never silently lost — into a per-run suppression log. `harbinger doctor` reads that log so you can see exactly what got rejected and why.

> **Why this exists.** The default behaviour of [schema-validated patterns](harbinger-patterns) is to reject low-signal AI output before it reaches a finding. That's a useful default, but it turns the validator into a black box: when a hunter expected a finding to land and it didn't, there is no recourse. `doctor` is the recourse.

## Two modes

```bash
harbinger doctor            # list mode: 20 most recent runs + suppression counts
harbinger doctor <run-id>   # detail mode: every suppression in that run
harbinger doctor latest     # detail mode for the most recent run by mtime
```
...
Recent harbinger runs (most recent first):
...
View details with: harbinger doctor <run-id>   (or 'latest')
```
...
```text
$ harbinger doctor 20260425T021015Z-jwt_audit

=== harbinger doctor — run 20260425T021015Z-jwt_audit ===
Suppressions: 3
...
The full enum is defined at [`harbinger/bin/validate-schema`](https://github.com/ProwlrBot/CyberBox/blob/main/harbinger/bin/validate-schema) and is canonical.
...
1. **Read the raw output.** Is it nonsense, off-topic, or hallucinated? → **Model issue.** Rerun with a higher-tier model (`harbinger pattern <name>` already routes JSON-shaped patterns to Claude Sonnet / Opus, but you can override at the `ai` call site).
...
```
${HARBINGER_DIR:-~/.harbinger}/runs/<run-id>/
...
- **`No runs found under ...`** — you haven't run a schema-validated pattern yet. Try `harbinger pattern jwt_audit < some_token.txt` to populate one.
- **`run not found: <id>`** — the run-id you passed doesn't exist as a directory under `${HARBINGER_DIR}/runs/`. Use list mode (`harbinger doctor` with no arg) to find the right id, or pass `latest`.
...
- [harbinger Pattern Catalog](harbinger-patterns) — the schemas that drive validation
- [`harbinger/bin/validate-schema`](https://github.com/ProwlrBot/CyberBox/blob/main/harbinger/bin/validate-schema) — the Python helper that emits the structured `error_class` values
- [Supply-chain trust](trust) — same trust story as the rest of the harbinger pipeline
