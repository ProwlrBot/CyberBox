...
Get up and running with CyberBox in just a few minutes.
...
Format `prowlrbot/cybersandbox:${version}`, for example, to use version 1.0.0.150:

```bash
docker run --security-opt seccomp=unconfined --rm -it -p 8080:8080 ghcr.io/prowlrbot/cybersandbox:1.0.0.150
```

> If port 8080 is occupied, map to a different port:

```bash
docker run --security-opt seccomp=unconfined --rm -it -p 3000:8080 ghcr.io/prowlrbot/cybersandbox:latest
# Then access via http://localhost:3000
```

## Welcome

After running the Docker command, you'll see the CyberBox ASCII logo:
...
🚀 CyberBox Agent Sandbox Environment
