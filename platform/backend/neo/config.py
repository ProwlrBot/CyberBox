"""Central configuration — all knobs overridable via env or ~/.neo/config.yaml."""
...
def _home() -> Path:
    return Path(os.environ.get("NEO_HOME", str(Path.home() / ".neo"))).expanduser()
...
class Settings:
    ...
    port: int = int(os.environ.get("NEO_PORT", "8787"))
    ...
    tools: dict[str, str] = field(
        default_factory=lambda: {
            "subfinder": os.environ.get("NEO_SUBFINDER", "subfinder"),
            "httpx": os.environ.get("NEO_HTTPX", "httpx"),
            "nuclei": os.environ.get("NEO_NUCLEI", "nuclei"),
        }
    )

    @property
    def db_path(self) -> Path:
        return self.home / "neo.db"
