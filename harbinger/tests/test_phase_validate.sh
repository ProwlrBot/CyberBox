...
#   `invoke-claude` by intercepting them via a temporary HARBINGER_BIN override
...
#
# Run: bash harbinger/tests/test_phase_validate.sh
...
mkdir -p "${HARBINGER_DIR}/targets/${TARGET_EMPTY}"/{recon,findings,evidence,reports,logs}
: > "${HARBINGER_DIR}/targets/${TARGET_EMPTY}/findings/nuclei.jsonl"
...
TARGET="testcorp.com"
WS="${HARBINGER_DIR}/targets/${TARGET}"
...
WS_FAIL="${HARBINGER_DIR}/targets/${TARGET_FAIL}"
