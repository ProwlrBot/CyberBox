#!/usr/bin/env bash
# Shared helpers for prowl pattern tests.
# Sourced by prowl/tests/test_pattern_<name>.sh

set -uo pipefail

PATTERN_HELPER_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROWL_BIN_DIR="${PATTERN_HELPER_DIR}/../bin"
PROWL_PATTERNS_DIR="${PROWL_PATTERNS_DIR:-${PATTERN_HELPER_DIR}/../patterns}"
...
# Picks the available backend and pipes the input through `prowl pattern <name>`.
...
invoke_pattern() {
  ...
  if [[ "$backend" == "ollama" ]]; then
    ...
    OLLAMA_MODEL="$ollama_model" "${PROWL_BIN_DIR}/harbinger" pattern "$name" < "$input_file" 2>/dev/null
  else
    "${PROWL_BIN_DIR}/harbinger" pattern "$name" < "$input_file" 2>/dev/null
  fi
}
...
run_pattern_test() {
  local name="$1"
  local pattern_dir="${PROWL_PATTERNS_DIR}/${name}"
  local fail=0
  ...
  echo "  INFO: invoking backend=${backend} via 'prowl pattern ${name}'"
