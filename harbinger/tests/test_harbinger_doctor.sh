#!/usr/bin/env bash
# Tests for `harbinger doctor` — schema validation suppression reporter.
# Run: bash harbinger/tests/test_harbinger_doctor.sh
set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
HARBINGER_BIN_DIR="${SCRIPT_DIR}/../bin"
HARBINGER="${HARBINGER_BIN_DIR}/harbinger"
VALIDATE_SCHEMA="${HARBINGER_BIN_DIR}/validate-schema"
...
out=$("$HARBINGER" doctor 2>&1) && rc=0 || rc=$?
if [[ "$rc" -eq 0 ]]; then
...
if echo "$out" | grep -qiE 'no runs|no harbinger runs|no runs yet'; then
...
RUN_DIR="${HARBINGER_RUNS_DIR}/${RUN_ID}"
mkdir -p "$RUN_DIR"
...
out=$("$HARBINGER" doctor "$RUN_ID" 2>&1) || true
...
out=$("$HARBINGER" doctor latest 2>&1) || true
if echo "$out" | grep -q "$RUN_ID"; then
...
out=$("$HARBINGER" doctor "no-such-run-id-xyz" 2>&1) && rc=0 || rc=$?
if [[ "$rc" -ne 0 ]]; then
...
CLEAN_DIR="${HARBINGER_RUNS_DIR}/${CLEAN_RUN_ID}"
mkdir -p "$CLEAN_DIR"
: > "${CLEAN_DIR}/suppression.jsonl"

out=$("$HARBINGER" doctor "$CLEAN_RUN_ID" 2>&1) && rc=0 || rc=$?
if [[ "$rc" -eq 0 ]]; then
